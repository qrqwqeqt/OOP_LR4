...
void Toolbar::OnNotify(HWND hWnd, LPARAM lParams) {
	...
	if (pnhm->code == TTN_NEEDTEXT)
	{
		...
		int bufferSize = MultiByteToWideChar(CP_UTF8, 0, pText, -1, NULL, 0);
		wchar_t* buffer = new wchar_t[bufferSize];
		MultiByteToWideChar(CP_UTF8, 0, pText, -1, buffer, bufferSize);

		lstrcpyW(lpttt->szText, buffer);
		...
		if (pnhm->code == TTN_NEEDTEXT)
		{
			...
			lstrcpyW(lpttt->szText, reinterpret_cast<LPCWSTR>(pText));
		}
	}
}
